...
require 'DouyinPayException.php';
...
class BaseService
{
    ...
    static $GATEWAY = "https://api.douyinpay.com";
    ...
    private function loadPlatformCertificate()
    {
        if (file_exists($this->platformCertificateFilePath)) {
            ...
            if ($this->platformPublicKeyInstance) {
                ...
                throw new Exception("抖音支付平台证书错误");
            }
        } else {
    ...
    {
        $result = $this->execute('GET', '/v1/merchant/certificates/getPlatformCertificates');
        $effective_time = 0;
        foreach ($result['certificates'] as $item) {
        ...
        $certificate = $this->decryptToString($encert['cipher_text'], $encert['nonce'], $encert['associated_data'] ?? '');
        if (!$certificate) {
            throw new Exception('抖音支付平台证书解密失败');
        }
        if (!file_put_contents($this->platformCertificateFilePath, $certificate)) {
            throw new Exception('抖音支付平台证书保存失败，可能无文件写入权限');
        }
        //从证书解析公钥与序列号
    ...
    public function execute(string $method, string $path, array $params = [], bool $cert = false)
    {
        ...
        $header[] = 'Authorization: DouyinPay-RSA ' . $authorization;
        if ($cert) {
            $header[] = 'Douyinpay-Serial: ' . $this->platformCertificateSerial;
        }
        ...
        if ($httpCode >= 200 && $httpCode <= 299) {
            if ($path != '/v1/merchant/certificates/getPlatformCertificates' && !$this->checkResponseSign($response, $header)) {
                throw new Exception("抖音支付返回数据验签失败");
            }
            return $result;
        }
        throw new DouyinPayException($result, $httpCode);
    }
    ...
    protected function checkResponseSign(string $body, string $header): bool
    {
        ...
        if (preg_match('/Douyinpay-Signature: (.*?)\r\n/i', $header, $signature)) {
            $signature = $signature[1];
        }
        if (preg_match('/Douyinpay-Nonce: (.*?)\r\n/i', $header, $nonce)) {
            $nonce = $nonce[1];
        }
        if (preg_match('/Douyinpay-Timestamp: (.*?)\r\n/i', $header, $timestamp)) {
            $timestamp = $timestamp[1];
        }
        if (preg_match('/Douyinpay-Serial: (.*?)\r\n/i', $header, $serial)) {
            $serial = $serial[1];
        }
    ...
    {
        $inDouyinpaySignature = $_SERVER['HTTP_DOUYINPAY_SIGNATURE'];
        $inDouyinpayTimestamp = $_SERVER['HTTP_DOUYINPAY_TIMESTAMP'];
        $inDouyinpaySerial = $_SERVER['HTTP_DOUYINPAY_SERIAL'];
        $inDouyinpayNonce = $_SERVER['HTTP_DOUYINPAY_NONCE'];
        $inBody = file_get_contents('php://input');

        if (empty($inBody)) {
            throw new Exception('no data');
        }
        if ($this->platformCertificateSerial != $inDouyinpaySerial) {
            if (!$this->download_cert) {
                $this->downloadCertificate();
            }
            if ($inDouyinpaySerial != $this->platformCertificateSerial) {
                throw new Exception('平台证书序列号不匹配');
            }
        }

        // 使用平台API证书验签
        if (!$this->checkSign($inDouyinpayTimestamp, $inDouyinpayNonce, $inBody, $inDouyinpaySignature)) {
    ...
    protected function curl(string $method, string $url, array $header, $body = null, int $timeout = 10): array
    {
        ...
        $ua = "douyinpay-php/" . self::$VERSION . " curl/" . $curlVersion['version'] . " (" . PHP_OS . "/" . php_uname('r') . ") PHP/" . PHP_VERSION;
