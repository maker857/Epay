...
case 'edit_pwd':
	...
	if(!empty($userrow['pwd']) && !\lib\PasswordHasher::verifyMerchant($oldpwd, $userrow['pwd'], $uid)){
	...
	$pwd = \lib\PasswordHasher::hash($newpwd);
	$sqs=$DB->update('user', ['pwd'=>$pwd], ['uid'=>$uid]);
	if($sqs!==false){
...
case 'refund_submit': //确认退款
	...
	if(!\lib\PasswordHasher::verifyMerchant($pwd, $userrow['pwd'], intval($userrow['uid'])))
