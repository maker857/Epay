...
if(isset($_GET['kw']) && !empty($_GET['kw'])) {
	...
	if(in_array((int)$_GET['type'], [1,2,4,5,6,7,8,9,10,11], true)) $params[':kw'] = $kw;
}

$file="系统订单号,商户订单号,接口订单号,商户ID,网站域名,商品名称,订单金额,实际支付,商户分成,支付方式,支付账号,支付IP,创建时间,完成时间,支付状态,已退款金额,退款时间\r\n";

$rs = $DB->query("SELECT A.*,B.plugin,C.apply_id submchid FROM pre_order A LEFT JOIN pre_channel B ON A.channel=B.id LEFT JOIN pre_subchannel C ON A.subchannel=C.id WHERE{$sql} order by trade_no desc limit 100000", $params);
while($row = $rs->fetch()){
	if($row['status']==2){
...
if(isset($_GET['kw']) && !empty($_GET['kw'])) {
	...
	if($_GET['type']==1){
		...
		$sql.=" AND A.`title` like :kw_like";
		$params[':kw_like'] = '%'.$kw.'%';
	}elseif($_GET['type']==5){
		$sql.=" AND A.`content` like :kw_like";
		$params[':kw_like'] = '%'.$kw.'%';
	}elseif($_GET['type']==6){
		$sql.=" AND A.`phone`=:kw";
	}
	if(in_array((int)$_GET['type'], [1,2,3,6], true)) $params[':kw'] = $kw;
}

$file="ID,支付方式,商户ID,关联订单号,商品名称,订单金额,问题类型,投诉原因,投诉详情,创建时间,最后更新时间,状态\r\n";

$rs = $DB->query("SELECT A.*,B.money,B.name ordername,C.apply_id submchid FROM pre_complain A LEFT JOIN pre_order B ON A.trade_no=B.trade_no LEFT JOIN pre_subchannel C ON A.subchannel=C.id WHERE{$sql} order by A.addtime desc limit 100000", $params);
while($row = $rs->fetch()){
...
if(isset($_GET['kw']) && !empty($_GET['kw'])) {
	...
	if($_GET['type']==1){
		...
		$sql.=" AND CAST(A.`money` AS VARCHAR) like :kw_like";
		$params[':kw_like'] = '%'.$kw.'%';
	}
	if(in_array((int)$_GET['type'], [1,2], true)) $params[':kw'] = $kw;
}

$file="系统订单号,分账规则,支付方式,订单金额,分账金额,时间,分账状态\r\n";

$rs = $DB->query("SELECT A.*,C.id channelid,C.name channelname,C.type,D.realmoney ordermoney FROM pre_psorder A LEFT JOIN pre_psreceiver B ON A.rid=B.id LEFT JOIN pre_channel C ON B.channel=C.id LEFT JOIN pre_order D ON D.trade_no=A.trade_no WHERE{$sql} order by A.id desc limit 100000", $params);
while($row = $rs->fetch()){
