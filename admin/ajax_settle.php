...
break;
case 'paypwd_check':
	if(isset($_SESSION['paypwd_verified']) && hash_equals($_SESSION['paypwd_verified'], hash_hmac('sha256', (string)$conf['admin_paypwd'], SYS_KEY)))
		exit('{"code":0,"msg":"ok"}');
	else
...
case 'paypwd_input':
	...
	if(\lib\PasswordHasher::verifyAdmin($paypwd, $conf['admin_paypwd'])){
		$_SESSION['paypwd_verified'] = hash_hmac('sha256', (string)$conf['admin_paypwd'], SYS_KEY);
		exit('{"code":0,"msg":"ok"}');
	}else{
...
case 'transfer':
	...
	if(!isset($_SESSION['paypwd_verified']) || !hash_equals($_SESSION['paypwd_verified'], hash_hmac('sha256', (string)$conf['admin_paypwd'], SYS_KEY)))exit('{"code":-1,"msg":"支付密码错误，请返回重新进入该页面"}');
