...
class Merchant
{
    public static function info(){
        ...
        $order_today_all = round($DB->getColumn("SELECT sum(money) FROM pre_order WHERE uid=:pid AND status=1 AND date=:today", [':pid'=>$pid, ':today'=>$today]),2);
	    $order_lastday_all = round($DB->getColumn("SELECT sum(money) FROM pre_order WHERE uid=:pid AND status=1 AND date=:lastday", [':pid'=>$pid, ':lastday'=>$lastday]),2);
